...
;EUCRECT (EUCLIDEAN RECTANGULE) IS NOT A GOOD NAME FOR THIS FUCNTION,
;BECAUSE IT PAINTS AN XZ/YZ ISOMETRIC RECTANGLE, BUT LIKE WE ALREADY HAVE
;A FUNCTION CALLED ISORECT THIS NAME IS GOOD FOR NOW.
...
DREGIONYZ:
	...
	LD	(ISODIR),A
	LD	DE,(TILE)
	LD	(R.CORNERL),DE		;CALCULATE LEFT CORNER (X)

RYZ.1:	DEC	E
	DEC	D
	DJNZ	RYZ.1
	LD	(R.CORNERU),DE		;CALCULATE UPPER RIGTH CORNER (J)

	LD	A,E
	ADD	A,C
	LD	E,A
	LD	(R.CORNERR),DE		;CALCULATE LOWER RIGTH CORNER (K)
	...
	ADD	A,C
	LD	E,A
	LD	(R.CORNERD),DE		;CALCULATE LOWER CORNER (Y)
	...
	;TODO: CHANGE CORNER NAMES

	LD	BC,(TILEINC)		;DRAW TILES U
	LD	DE,(R.CORNERL)
	DEC	B
	DEC	D
	DEC	E
	...
	LD	DE,(R.CORNERL)		;DRAW TILE X
	LD	A,MASKRGHTUP2
	LD	(MASK),A
	LD	HL,DRAWTILEM
	CALL	DRAWTILEM

	LD	DE,(R.CORNERU)		;DRAW TILE J
	LD	A,MASKRGHTUP3
	LD	(MASK),A
	LD	HL,DRAWTILEM
	CALL	DRAWTILEM
	...
	LD	DE,(R.CORNERR)		;DRAW TILE K
	LD	A,MASKLEFTDW2
	LD	(MASK),A
	LD	HL,DRAWTILEM
	CALL	DRAWTILEM

	LD	DE,(R.CORNERD)		;DRAW TILE Y
	LD	A,MASKLEFTDW3
	LD	(MASK),A
	LD	HL,DRAWTILEM
	CALL	DRAWTILEM
...
MARKREGION:
	LD	A,DEFARG		;REGION ARE MARKED USING RECTANGULES
	LD	(CMDARG),A
	XOR	A
	LD	(ISODIR),A
	LD	BC,(TILEINC)
	...
	LD	A,(ISOTYPE)
	CP	ISOXY
	JR	Z,ISORECT		;ISORECT IN THE CASE OF XY REGIONS
	CP	ISOXZ
	JP	Z,EUCRECT		;EURECT IN THE CASE OF XZ OR YZ

	LD	A,1			;YZ IS THE SAME THAN XZ BUT WITH
	LD	(ISODIR),A		;NEGATIVE MOVEMENT
	JP	EUCRECT
