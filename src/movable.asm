...
;INPUT:	IX = POINTER TO THE MOVABLE
	...
	CSEG
	EXTRN	MOV16ISO,HEIGTH,GETRINFO,ALLOCMOB

ANIMMOV:LD	A,(IX+MOV.DIR)
	LD	L,(IX+MOV.XR)
	LD	H,(IX+MOV.XR+1)
	LD	E,(IX+MOV.YR)
	LD	D,(IX+MOV.YR+1)
	...
	LD	(IX+MOV.YR),E
	LD	(IX+MOV.YR+1),D
	CALL	DRAW			;DRAW NEW MOVABLE POSITION
	DEC	(IX+MOV.STEPCNT)
	RET	NZ

	CALL	UNLINKANIM		;UNLINK FROM ANIMATED MOVABLES
	LD	L,(IX+MOV.RINFO)	;REMOVE THE MOVABLE FROM PREVIOUS
	LD	H,(IX+MOV.RINFO+1)	;POSITION
	LD	A,L
	OR	H
	JR	Z,A.DST
...
A.DST:	LD	E,IXL
	...
	LD	A,-1
	CP	(IX+MOV.MOB)
	RET	NZ			;IT ALREADY HAS MOB, RETURN
	CALL	ALLOCMOB
	LD	(IX+MOV.MOB),A
	RET
...
S.OK:	LD	A,(S.DIR)		;COPY DESTINE TO THE MOVABLE
	LD	(IX+MOV.DIR),A
