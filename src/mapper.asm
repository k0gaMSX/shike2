...
RUNMCMD:LD	A,(MAPCMD)
	...
	;WITH THIS LOCAL VARIABLE WE PRESERVE
...
CH.2:	LD	BC,(TILEINC)		;5,6 BYTES: TILEINC
	LD	(HL),C			;(4,5 IN MAPHOLE)
	INC	HL
	LD	(HL),B
	INC	HL
...
C.MPAT:	LD	BC,(METAPAT)		;7,8 (5,6 IN MAPTILE) BYTES: METAPAT
	LD	(HL),C
	INC	HL
	LD	(HL),B
	INC	HL
...
DE.2:	LD	C,(HL)			;5,6 BYTES: TILEINC
	INC	HL			;(4,5 BYTES IN MAPHOLE
	LD	B,(HL)
	INC	HL
	LD	(TILEINC),BC
...
D.MPAT:	LD	C,(HL)			;7,8 (5,6 IN MAPTILE) BYTES: METAPAT
	INC	HL
	LD	B,(HL)
	INC	HL
	LD	(METAPAT),BC
...
;INPUT:	DE = POINTER TO THE INPUT BUFFER
	...
	CSEG
	PUBLIC	MAP,MAP_
	EXTRN	CLRVPAGE,DISSCR,DISSPR,ENASCR,ENASPR,HMMM,VDPSYNC,SETPAGE
...
MA.PAG:	LD	(ACPAGE),A
	...
	LD	A,TILPAGE
	LD	(VDPPAGE),A
	CALL	HMMM			;COPY TILPAGE TO TILPAGE + 1

	LD	A,TILPAGE		;SET THE CORRECT PAGES
	LD	(DPPAGE),A
	INC	A
	LD	(ACPAGE),A
	CALL	SETPAGE
	...
	;MAP_ DOES ONLY THE CENTRAL PART OF MAP, IT DOESN'T CLEAN OR COPY
	;ANYTHING ELSE
