...
I.KBD:	DW	KEYBOARD
	DB	0,0,0
I.DUMMY:DW	DUMMY
	DB	MAXISOX-1,MAXISOY-1,0


;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;INPUT:	HL = CHAR DEFINITION:
...
N.FOUND:LD	(IX+CHAR.LEVEL),0	;REMOVE FROM FREE LIST
	LD	A,(N.MOB)
	...
	LD	E,(HL)
	INC	HL
	LD	D,(HL)			;DE = CONTROLLER
	INC	HL
	LD	(IX+CHAR.CONTROL),E	;ASSIGN CONTROLLER
	LD	(IX+CHAR.CONTROL+1),D
	LD	E,(HL)			;E = Y COORDENATE
	INC	HL
	LD	D,(HL)			;D = X COORDENATE
	INC	HL
	LD	A,(HL)			;A = Z COORDENATE
	LD	(IX+CHAR.X),D
	LD	(IX+CHAR.Y),E
	LD	(IX+CHAR.Z),A
