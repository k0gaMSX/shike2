...
IR.LOOP:
	...
	CALL	ISORECT			;PAINT THE RECTANGULE REGION
	CALL	VDPSYNC			;WAIT UNTIL THE END OF PAINTING
	CALL	GETCHAR			;WAIT A KEY
	PUSH	AF
	LD	BC,(IR.INCS)
	CALL	ISORECT			;DELETE PREVIOUS REGION
	LD	BC,(IR.INCS)
...
.P1UP:	CP	KB_UP
	...
	LD	A,C
	OR	A
	JR	Z,IR.LOOP		;WE CAN NOT GO MORE UP THAN ORIGIN
	DEC	C
	JR	IR.LOOP
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;INPUT:		(TILE) = ACTUAL TILE POSITION
;		B = X SIZE OF THE RECTANGULE
;		C = Y SIZE OF THE RECTANGLE
	CSEG
...
ISORECT:
	...
	;         LOL
	;          L
	;SO THE FIRST LOOP (B = 2) DRAWS THE L PATTERNS, WHILE SECOND LOOP
	;(B = 1) DRAWS THE O PATTERNS. YOU CAN SEE THAT THE SECOND LOOP
	;IS ALWAYS ONE PATTERN LESS IN X AND Y.

I.LOOP2:
	PUSH	BC
	LD	BC,(I.TILEINC)		;LOAD YINC IN B
	LD	B,C
	INC	B
	JR	Z,I.ELOOP2		;SKIP THE LOOP BECAUSE Y=0

I.LOOP1:
	PUSH	DE			;LOOP OVER Y AND DRAW PARAREL ISO LINES
	PUSH	BC
	LD	BC,(I.TILEINC)		;LOAD XINC IN B
	INC	B
	JR	Z,I.ELOOP1		;SKIP THE LOOP BECAUSE X=0
...
I.ELOOP2:
	POP	BC			;BEGIN THE SECOND LOOP
	LD	DE,(I.TILEINC)		;DECREMENT THE X AND Y SIZES
	DEC	E
	DEC	D
	LD	(I.TILEINC),DE
	LD	DE,(I.TILE)		;AND BEGIN WITH THE TILE IN THE RIGHT
	INC	D			;OF THE FIRST
	LD	(I.TILE),DE
	DJNZ	I.LOOP2
