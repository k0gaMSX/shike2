...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;INPUT:		(MAPCMD) = ACTUAL MAPPER COMMAND
;		A = DIRECTION
;		DE = ACTUAL INCREMENT
;OUTPUT:	DE = INCREMENT AFTER MOVMENT

	CSEG
	EXTRN	MOVEUC,MOVIEUC,MOVIYEUC,MAPCMD

	;MOVEUC => NORMAL EUCLIDEAN MOVEMENT
	;MOVIEUC => INVERSE EUCLIDEAN MOVEMENT
	;MOVIYEUC => INVERSE Y EUCLIDEAN MOVEMENT

MOVINC:	EX	AF,AF'			;SELECT THE CORRECT MOVEMENT
	LD	A,(MAPCMD)		;FUNCTION BASED IN THE MAPPER
	ADD	A,A			;COMMAND WE ARE RUNNING
	LD	HL,MI.DATA
	ADD	A,L
	LD	L,A
	JR	NC,MI.1
...
MI.DATA:	DW	MOVEUC,MOVIEUC,MOVIYEUC,MOVIEUC,MOVIYEUC,MOVIEUC
