...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	CSEG
	PUBLIC	EDITOR
	EXTRN	ISOCMD,PATTERN,TILE,SPRITE,COLORSPRITE,EXIT
EDITOR:
	LD	C,TILEPAT
	LD	DE,TILEGEN
...
;INPUT:	(PATTERN) = ACTUAL SELECTED PATTERN
	...
	CSEG
	EXTRN	PAT2TILE,METAPAT,MARKMETAPAT,GETKEV
...
M.DOWN:	CP	KB_DOWN
	...
	LD	A,E
	ADD	A,C
	CP	15			;256 PATTERNS OF 16X8 GIVES 16 ROWS
	JR	Z,M.LOOP
	INC	C
	JR	M.LOOP
...
M.RGHT:	CP	KB_RIGTH
	...
	LD	A,D
	ADD	A,B
	CP	15			;256 PATTERNS OF 16X8 GIVES 16 COLUMNS
	JR	Z,M.LOOP
	INC	B
	JR	M.LOOP
...
;INPUT:	(PATTERN) = ACTUAL SELECTED PATTERN
	...
	CSEG
	EXTRN	DELSPR,SETPAGE,PATTERN,TILE2XY,PAT2XY,GETKEV,PUTSPRITE
...
P.LOOP:	LD	A,D
	...
	CP	KB_ESC
	JR	NZ,P.SPC		;LONGJMP TO EDITOR LOOP. CANCEL
...
;		(TILE) = ACTUAL TILE
	...
	CSEG
	EXTRN	SETPAGE,DELSPR,TILEINC,MARKREGION,GETKEV,VDPSYNC,NEXTISO
...
IR.LOOP:
	...
	POP	AF
	CP	KB_ESC
	JR	NZ,IR.SEL		;LONGJMP TO EDITOR LOOP. CANCEL
	LD	HL,(EDSTACK)		;ANY OPERATION
	LD	SP,HL
	JP	EDLOOP

IR.SEL:	CP	KB_SELECT		;CHANGE THE PLANE WHERE THE REGION
	JP	NZ,IR.SPC		;IS DRAWN
	CALL	NEXTISO
	JR	IR.LOOP
...
;INPUT:		(TILE) = ACTUAL TILE SELECTED
	...
	CSEG
	EXTRN	DELSPR,SETPAGE,TILE,ISOTILE,TILE2XY,GETKEV,PUTSPRITE,EXIT
