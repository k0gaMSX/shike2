...
;	BC = ROOM
	...
	CSEG
	PUBLIC	FOCUSCAM
	EXTRN	MOUSE,MAP,CLRVPAGE,CPVPAGE,GETHMAP,SHOWDOORS
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;INPUT:	IX = POINTER TO MOVABLE ORIGIN
;	IY = POINTER TO MOVABLE DESTINE
;OUTPUT:Z = 1 WHEN NO PATH

	CSEG
	PUBLIC	PATH2MOV
	EXTRN	MOVHMATRIX,ARYHL
...
PATH2MOV:
	...
	LD	(M.MATRIX),HL		;GET THE HEIGHT POINTER
	LD	IY,(M.MOV)		;IF WE ARE NEAR OF DESTINE DO NOTHING
	LD	E,(IY+MOV.Y)
	LD	D,(IY+MOV.X)
	LD	C,(IX+MOV.Y)
	LD	B,(IX+MOV.X)
	CALL	DISTANCE		;IF WE ARE NEAR OF DESTINE DO NOTHING
	CP	1
	JP	Z,T.BAD
	...
	LD	DE,M.POINT
	LD	BC,SIZPOINT
	LDIR				;COPY DESTINE POINT TO LOCAL VARIABLE
	...
	LD	DE,(M.POINT+POINT.Y)
	LD	(M.DST),DE
	LD	B,4
T.LOOP:	PUSH	BC			;TRY EACH DIRECTION
	LD	HL,M.INCS
	LD	A,C
	CALL	ARYHL
	LD	DE,(M.DST)		;GET THE MAP POSITION
	LD	A,E
	ADD	A,L
	LD	E,A
	LD	A,D
	...
	LD	HL,(M.MATRIX)
	LD	(M.POINT+POINT.Y),DE
	PUSH	HL
	CALL	MOFFSET
	LD	A,(HL)			;A = HEIGHT OF POSITION
	POP	HL
	LD	(M.POINT+POINT.Z),A

	LD	IY,M.POINT
	SUB	(IY+POINT.Z)		;A = HO - HD
	JR	NC,T.1
	NEG				;IF HO - HD < 0 THEN A = -A
T.1:	CP	1
	JR	NC,T.ELOOP		;IT IS A NO ACCESIBLE POSITION

	CALL	PSEARCH			;SEACH THE PATH
T.ELOOP:POP	BC
	JR	NZ,T.END		;NZ => GOOD PATH, END
	LD	A,C
	INC	A
	AND	3
	LD	C,A
...
T.BAD:	LD	A,SIZPATH		;WE HAVEN'T PATH, SIZE = 0
	LD	(OUTCNT),A
T.END:	POP	IX
	LD	A,(OUTCNT)
	LD	B,A
...
PSEARCH:LD	(P.MATRIX),HL
	...
	CALL	DISTANCE		;IT IS THE SAME POINT, RETURN
	RET	Z
...
P.NEXT:	CALL	PDEBUG
	CALL	GETNODE			;GET THE BEST NODE
	RET	Z
	INC	HL
	LD	A,(HL)
	INC	A
